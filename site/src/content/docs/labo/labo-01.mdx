...
Les colonnes **G, H et I** seront calculées par tes formules.
</Aside>
...
<Aside type="note">
  Tu auras besoin d'imbriquer des `SI()` pour gérer les trois cas ou alternativement tu peux utiliser `SI.CONDITIONS()`.   
  Pour vérifier si une cellule contient une date, tu peux la comparer à une chaîne vide : `F2<>""` retourne VRAI si une date est saisie.
</Aside>
...
<Aside type="tip">
  ...
  Par exemple : `TEXTE(A1;"# ##0,00 $")` transforme un nombre en texte avec le format monétaire.
</Aside>
